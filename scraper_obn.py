#!/usr/bin/env python3
"""PP Oberbayern Nord Scraper – alle Vorfälle"""
...
TITLE_MUST_CONTAIN = "oberbayern nord"
...
def detect_ort(text):
    # OBN: Ort steht oft als "Stadtname, Lkr. X" oder direkt vor " –"
    m=re.search(r'^([A-ZÄÖÜ][A-ZÄÖÜa-zäöüß/ -]+?)(?:,\s*Lkr\.|\s*[–-])',text)
    return m[1].strip() if m else "Unbekannt"
...
def get_urls():
    ...
    for page in range(MAX_PAGES):
        ...
        time.sleep(0.5)
    # Direkte Listenseite
    print("  🌐 polizei.bayern.de…")
    list_html=fetch("https://www.polizei.bayern.de/aktuelles/pressemitteilungen/index.html",timeout=15)
    if list_html:
        for m in re.finditer(r'href="(/aktuelles/pressemitteilungen/(\d{6})/index\.html)"',list_html):
...
def parse_article(html,url,from_date,to_date):
    ...
    if sections:
        for h in sections:
            ...
            incidents.append({"date":pm_date.strftime("%d.%m.%Y"),"dateSort":pm_date.strftime("%Y-%m-%d"),
                "time":parse_time(body),"nr":nr[1] if nr else "","kategorie":kat,"schweregrad":sev,
                "ort":ort,"pp":PP_NAME,"region":PP_NAME,"titel":titel[:120],"volltext":body[:1500],"link":url})
        return incidents

    # Format 2: Bold-Ort-Marker + Paragraphen (typisch OBN)
    ...
    for p in content.find_all("p"):
        ...
        # Ort-Marker erkennen: Bold-Text mit "Lkr." oder kurzer Ortsname
        if bold_text and (", Lkr." in bold_text or (len(bold_text)<50 and re.match(r'^[A-ZÄÖÜ]',bold_text) and len(bold_text.split())<6)):
            if collected:
                body=" ".join(collected)
                ...
                collected=[]
            current_ort=detect_ort(bold_text)
            if current_ort=="Unbekannt": current_ort=bold_text[:50]
        elif len(text)>30:
    ...
    # Fallback: gesamter Text als ein Vorfall
    if not incidents:
        full=content.get_text(" ",strip=True)
        if len(full)>50:
            kat,sev=categorize(full)
            incidents.append({"date":pm_date.strftime("%d.%m.%Y"),"dateSort":pm_date.strftime("%Y-%m-%d"),
                "time":"","nr":"","kategorie":kat,"schweregrad":sev,
                "ort":detect_ort(full),"pp":PP_NAME,"region":PP_NAME,
                "titel":title[:120],"volltext":full[:1500],"link":url})
    return incidents
