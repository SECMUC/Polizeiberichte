#!/usr/bin/env python3
"""PP Oberbayern Süd Scraper – alle Vorfälle"""
...
TITLE_MUST_CONTAIN = "oberbayern süd"
...
def parse_article(html,url,from_date,to_date):
    ...
    # Format 2: Bold-Ort-Marker + Paragraphen
    ...
    for p in content.find_all("p"):
        ...
        if bold_text and (", Lkr." in bold_text or (len(bold_text)<60 and re.match(r'^[A-ZÄÖÜ]',bold_text) and len(bold_text.split())<8)):
            if collected:
                body=" ".join(collected)
            ...
            current_ort=ort if ort!="Unbekannt" else bold_text[:50]
        elif len(text)>30:
